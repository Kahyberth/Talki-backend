...
export class LiveKitController {
  ...
  @Post('generate-token')
  async generateToken(@Body('roomName') roomName: string, @Body('userName') userName: string) {
    try {
      // Asegúrate de que el método generateAccessToken esté devolviendo un valor
      const token = await this.liveKitService.generateAccessToken(roomName, userName); // Usar await si es asíncrono
